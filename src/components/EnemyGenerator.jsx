...
const EnemyGenerator = ({
  ...
  const getSpawnRate = () => {
    return Math.max(minSpawnRate, baseSpawnRate - (spawnRateReduction * waveLevel));
  };
  ...
  const spawnEnemyBatch = () => {
    ...
    const actualBatchSize = Math.min(currentBatchSize, maxEnemies - enemies.length);
    ...
    for (let i = 0; i < actualBatchSize; i++) {
      ...
      const offsetX = actualBatchSize > 1 ? (Math.random() - 0.5) * batchSpread : 0;
