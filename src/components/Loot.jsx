    ...
    useFrame((state, delta) => {
      ...
      if (isBeingCollected) {
        ...
        if (animProgress.current < 1) {
          ...
          const easedProgress = easeOutQuad(animProgress.current * .5);
          
          // Move toward inventory UI
          const newX = THREE.MathUtils.lerp(startPos.current.x, targetPos.current.x, easedProgress);
          const newZ = THREE.MathUtils.lerp(startPos.current.z, targetPos.current.z, easedProgress);
          ...
          const directY = THREE.MathUtils.lerp(startPos.current.y, targetPos.current.y, easedProgress);
          ...
          itemRef.current.scale.set(scaleMultiplier, scaleMultiplier, scaleMultiplier);
          ...
          if (glowRef.current && glowRef.current.material) {
            glowRef.current.material.uniforms.intensity.value = 0.4 + easedProgress * 0.6;
            glowRef.current.material.uniforms.speed.value = 2.5 + easedProgress * 5;
          }
        } else {
        ...
        time.current += delta;
        itemRef.current.position.y = position[1] + Math.sin(time.current * 2) * 0.1 - 0.1;
        itemRef.current.rotation.y += delta * 0.8;
        
        clonedScene.traverse((child) => {
          if (child.isMesh && child.material) {
            child.material.emissiveIntensity = 0.051 + Math.sin(time.current * 3) * 0.051;
          }
        });
      }
      ...
      <group position={position}>
        <group ref={itemRef} scale={0.8}>
          ...
          <Html
            ...
            distanceFactor={10}
            style={{ 
              opacity: isBeingCollected ? Math.max(0, 1 - animProgress.current * 2) : 1 
            }}
          >
            <div style={{
        ...
        {isBeingCollected && animProgress.current < 0.7 && (
          <ParticleEffect position={itemRef.current.position} progress={animProgress.current} />
        )}
      </group>
    );
  });
  ...
  useFrame((state, delta) => {
    if (particles.current) {
      ...
      for (let i = 0; i < particles.current.children.length; i++) {
        ...
        particle.position.addScaledVector(particle.userData.direction, delta * 0.5);
...
const Loot = () => {
  ...
  const [loot, pickupLoot] = useGameEffectsStore(
