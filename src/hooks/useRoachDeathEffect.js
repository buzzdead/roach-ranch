...
const useRoachDeathEffect = ({
    ...
    if (isDead && deathProgress.current < 1) {
      const animateDeath = () => {
        ...
        const pulseIntensity = newProgress < 0.5 ?
          Math.sin(newProgress * 20) * 0.3 + 1 :
          Math.sin(newProgress * 20) * 0.2 * (1 - newProgress) + 1;

        materialsRef.current.forEach((material, index) => {
          const individualProgress = Math.min(newProgress * (1 + (index % 3) * 0.1), 1);
          if (individualProgress > 0.4) {
            material.opacity = 1 - ((individualProgress - 0.4) / 0.6);
          }
        ...
        if (modelRef.current) {
          let scale = newProgress < 0.3 ?
            1.25 * (1 + newProgress * 0.07) :
            1.25 * (1.07 - ((newProgress - 0.3) / 0.7) * 1.07);
        ...
        if (newProgress >= 0.99 && onDeathComplete) {
      ...
      if (!startedDeathAnimation.current) {
  ...
  return { deathProgress }; // Expose progress if needed
};
